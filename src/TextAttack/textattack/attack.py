...
class Attack:
    ...
    def __init__(
        ...
        self.search_method.get_transformations_batch_wise = self.get_transformations_batch_wise
        ...
        self.search_method.get_indices_to_order_batchwise = self.get_indices_to_order_batchwise
    ...
    def get_indices_to_order_batchwise(self, list_current_text, **kwargs):
        """Applies ``pre_transformation_constraints`` to ``text`` to get all
        the indices that can be used to search and order.
        ...
        Returns:
            The length and the filtered list of indices which search methods can use to search/order.
        """
        batch_indices_to_order = []
        batch_len_text = []
        for current_text in list_current_text:
            ...
            batch_indices_to_order.append(list(indices_to_order))

            len_text = len(list(indices_to_order))
            batch_len_text.append(len_text)

        # Convert indices_to_order to list for easier shuffling later
        return batch_len_text,  batch_indices_to_order
    ...
    def _get_transformations_uncached_batch_wise(self, current_text, original_text=None, **kwargs):
        """Applies ``self.transformation`` to ``text``, then filters the list
        of possible transformations through the applicable constraints.
        ...
        transformed_texts, unsuccesful_mask,synonyms = self.transformation(
            current_text,
            pre_transformation_constraints=self.pre_transformation_constraints,
            **kwargs,
        )
    ...
    def get_transformations(self, current_text, original_text=None, **kwargs):
        ...
        if self.use_transformation_cache:
            ...
            if utils.hashable(cache_key) and cache_key in self.transformation_cache:
                ...
                if isinstance(transformed_texts,dict):
                    ValueError('not expecting dictionary')
        ...
        # print ('transformed',transformed_texts)
        a = time.time()
        
        if isinstance(transformed_texts,dict):
            transformed_texts['transformed_texts'] = self.filter_transformations( transformed_texts['transformed_texts'], current_text, original_text)
            filtered = transformed_texts
        else:

            filtered= self.filter_transformations(
                transformed_texts, current_text, original_text
            )
        # print ('time to filter',time.time() - a)
    
        return filtered

    def get_transformations_batch_wise(self, current_text, original_text=None, **kwargs):
        """Applies ``self.transformation`` to ``text``, then filters the list
        of possible transformations through the applicable constraints.
        ...
        if self.use_transformation_cache: # this won't work anymore only uncache method works
            cache_key = tuple([current_text] + sorted(kwargs.items()))
            if utils.hashable(cache_key) and cache_key in self.transformation_cache:
                # promote transformed_text to the top of the LRU cache
                self.transformation_cache[cache_key] = self.transformation_cache[
                ...
                transformed_texts ,unsuccessful_mask,synonyms= self._get_transformations_uncached_batch_wise(
                    current_text, original_text, **kwargs
                )
                if utils.hashable(cache_key):
                    self.transformation_cache[cache_key] = tuple(transformed_texts)
        ...
        a = time.time()
        filtered_list = []
        print ('transformed_texts',len(transformed_texts))
        for b in range(len(transformed_texts)):
            # print ('transformed_texts b',transformed_texts[b],current_text[b],original_text[b])
            filtered= self.filter_transformations(
                transformed_texts[b], current_text[b], original_text[b]
            )
            # print ('filtered',filtered)
            filtered_list.append(filtered)
            
        print ('filtered list',len(filtered_list))
        return filtered_list, unsuccessful_mask,synonyms
    ...
    def _attack_batch_wise(self, initial_result_gfr):
        """Calls the ``SearchMethod`` to perturb the ``AttackedText`` stored in
        ``initial_result``.
        ...
        """ 
        # print ('initial_result_gfr',initial_result_gfr)
        final_result = self.search_method(initial_result_gfr)

        # print ('final result',final_result)
        
        # {'goal_function_result':goal_function_result_list,'skipped_sample_mask':skipped_sample_mask}
        initial_result = initial_result_gfr['goal_function_result'] 
        self.clear_cache()
        result_list = []
        for b in range(len(final_result)):
            if final_result[b].goal_status == GoalFunctionResultStatus.SUCCEEDED:
                # print ('one sample successful')
            ...
            elif final_result[b].goal_status == GoalFunctionResultStatus.SKIPPED:
                
                result = SkippedAttackResult(
                    initial_result[b]
                )
                result_list.append(result)
            elif final_result[b].goal_status == GoalFunctionResultStatus.SEARCHING:
                result = FailedAttackResult(
                    initial_result[b],
                    final_result[b],
                )
                result_list.append(result)
            elif final_result[b].goal_status == GoalFunctionResultStatus.MAXIMIZING:
                result = MaximizedAttackResult(
                    initial_result[b],
                    final_result[b],
                )
                ...
                raise ValueError(f"Unrecognized goal status {final_result[b].goal_status}")
        # sys.exit()
        return result_list
    ...
    def attack(self, example, ground_truth_output):
        ...
        if goal_function_result.goal_status == GoalFunctionResultStatus.SKIPPED:
            ...
            # goal_function_result = [goal_function_result,goal_function_result]
            result = self._attack(goal_function_result)
            return result
    
    def attack_batch_wise(self, example, ground_truth_output):
        """Attack a single example.
        ...
        for b in range(len(example)):
            assert isinstance(
                example[b], (str, OrderedDict, AttackedText)
            ), "`example` must either be `str`, `collections.OrderedDict`, `textattack.shared.AttackedText`."
            if isinstance(example[b], (str, OrderedDict)):
                example[b] = AttackedText(example) 

        # assert isinstance(
        #     ground_truth_output, (int, str)
        # ), "`ground_truth_output` must either be `str` or `int`."
        goal_function_result_list = []
        skipped_sample_mask = []
        for b in range(len(example)):

            goal_function_result, _ = self.goal_function.init_attack_example(  example[b], ground_truth_output[b] )
            goal_function_result_list.append(goal_function_result)

            if goal_function_result.goal_status == GoalFunctionResultStatus.SKIPPED:
                # goal_function_result[b] = SkippedAttackResult(goal_function_result)
                skipped_sample_mask.append(False)
            else:
                skipped_sample_mask.append(True)
 
        result = self._attack_batch_wise({'goal_function_result':goal_function_result_list,'skipped_sample_mask':skipped_sample_mask})
        return result
