...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_space", default="adv_data_size_plot", type=str, help="where to save the tests")
    ...
    for M in method_test:
        ...
        if M == 'OT':
            ...
            Data_ratios_colors = {0.05:'r',0.1:'b',0.2:'g',0.5:'y',1.0:'m'}
        ...
        for i,LR in enumerate(Learning_rate):
            ...
            for j,Lda in enumerate(Lambdas):
                for k,Data_ratio in enumerate(Data_ratios):
                    ...
                    plot_color = Data_ratios_colors[Data_ratio]
                    ...
                    def magnify(fig,ax1,ax2,ax3,ax4,Epochs,trend):
                        ...
                        if trend:
                            ...
                            ticks = 10
                            label = None
                            label_trend = f'{Data_ratio*100}%'
                        else:
                            alpha = 1
                            ticks = 1
                            label = f'{Data_ratio*100}%'
                            label_trend = None


                        fig.suptitle(f"Robust performance over\nadversarial dataset size",fontsize=21)
                        ...
                        if trend:
                            ...
                            linestyle='dashed',
                            color = plot_color,label=label_trend )
                        ...
                        png_folder = f'{args.save_space}/{output_dir}/{M}/Pictures/PNG/'
                        eps_folder = f'{args.save_space}/{output_dir}/{M}/Pictures/ESP/'

                        os.makedirs(os.path.dirname(png_folder), exist_ok=True)
                        os.makedirs(os.path.dirname(eps_folder), exist_ok=True)


                        T_Save_Name = f"{Id}_D{Dataset}_AKT{AttackTrain}_ATE{AttackEvaluate}_vs_all_plot_over_epochs_E{sum(Epochs)}_AX4{ax4_name}"
