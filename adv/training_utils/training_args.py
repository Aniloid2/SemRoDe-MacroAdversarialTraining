...
class TrainingArgs:
    ...
    Dist_Val: float = 1.0
    FLB_Val: float = 1.0
    Online_AT_Val: str = None 
    Method_Dictionary: Dict = field(default_factory=default_method_dictionary)

    Data_ratio: float = 0.2
    Dataset_attack: str = 'MR'
    model_name:str = 'BERT'
    Sem_Sim: float = None
    Cos_Sim: float = None
    No_Cand: int = None
    max_modification_rate_adv: float = 0.3 

    AttackTrain: str = 'TextAttack'
    AttackEvaluate: str = 'TextAttack'
    ...
    def _add_parser_args(cls, parser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=default_obj.log_to_tb,
            help="If set, log to CSV",
        )
        parser.add_argument(
            "--csv-log-dir",
            type=str,
            default=default_obj.csv_log_dir,
            help="Path of CSV log directory.",
        )
        parser.add_argument(
            "--log-to-wandb",
            action="store_true",
        ...
        parser.add_argument(
            "--AT-Val",
            type=flaot,
            help="Adversarial training constant",
        )
        parser.add_argument(
            "--OT-Val",
            type=flaot,
            help="OT training constant",
        )
        parser.add_argument(
            "--Data-ratio",
            type=flaot,
            help="Data ratio to attack",
        )
        parser.add_argument(
            "--Dataset_attack",
            type=string,
            help="Data set to attack",
        )
