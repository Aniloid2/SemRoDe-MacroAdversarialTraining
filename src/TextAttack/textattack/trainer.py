...
from .attack_results import MaximizedAttackResult, SuccessfulAttackResult, FailedAttackResult, SkippedAttackResult
...
class Trainer:
    ...
    def training_step(self, model, tokenizer, batch,adv_batch,progress_batch,progress_extra_batch,pos_base_batch,neg_base_batch,pos_adv_batch,neg_adv_batch):
                ...
                if self.training_args.Method_Dictionary['Embedding'] == 'ASCC': 
                    class ASCCTrainer(EmbeddingLevelGradientTrainer):
                        ...
                        def train(self, args, batch: Tuple) -> float:
                            ...
                            print (batch[0][0])
                            batch_decoded = self.tokenizer.decode(batch[0][0], skip_special_tokens=True)
                            print (batch_decoded) 
        ...
        return (loss, preds, _targets,(s_loss , da_loss,adv_loss,mmd_loss,l2_loss,mmd_progress_extra_loss,crl_loss,total_flb_loss,pos_adv_da_loss,neg_adv_da_loss )) if s_loss is not None else (loss, preds, _targets,None)
