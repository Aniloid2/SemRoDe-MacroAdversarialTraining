    ...
    def forward(self, input_ids, attention_mask=None, head_mask=None):
        ...
        if self.training:
            ...
            if self.adver_type == 'ot':
                ...
                # print ('batch size',batch_size,'num_dim',num_dim,'num head',num_head,'num_key',num_key)

                query_layer = query_layer.reshape([-1, num_key, num_dim])
                key_layer = key_layer.reshape([-1, num_key, num_dim])
                # print ('training shapes query key',query_layer.shape,key_layer.shape)

                # print ('qulay',query_layer,query_layer.shape)
                # print ('klay',key_layer,key_layer.shape)
                #
                # sys.exit()
            ...
            # MMD loss calclation
            mmdloss = MMD_loss().cuda()
            self.MMD_backward = mmdloss(query_layer, key_layer.detach()).mean().detach()


            # OT distance calculation based on l2 distance i suppose
            batch_size = key_layer.shape[0]
            num_dim = key_layer.shape[-1]
            num_head = key_layer.shape[1]
            num_key= key_layer.shape[2]
            ...
            # self.KL_backward = torch.Tensor(1)
    ...
    Returns:
        ...
        for inner_list in self.albert.encoder.KL_list:
            for item in inner_list:
                ...
                count = count + 1
        KL = KL / count
        # print ('kl layer space',self.albert.encoder.KL_list)
        # print ('mmd layer space',self.albert.encoder.MMD_list)
        ...
        # print ('output sequence classification')
