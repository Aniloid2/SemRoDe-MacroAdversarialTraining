...
class MMD_loss(nn.Module):
    def __init__(self, kernel_mul = 4.0, kernel_num = 10):
    ...
    def guassian_kernel(self, source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
            ...
            # print ('bandwith scaled',bandwidth)
            bandwidth_list = [bandwidth * (kernel_mul**i) for i in range(kernel_num)]
            # print ('bad list',bandwidth_list)
            kernel_val = [torch.exp(-L2_distance / bandwidth_temp) for bandwidth_temp in bandwidth_list]
            # print ('kernel_val',len(kernel_val))
...
class AlbertAttention(nn.Module):
    def __init__(self, config: AlbertConfig):
        ...
        try:
            ...
            self.att_se_hid_size = None

        try:
            self.att_se_nonlinear  = config.att_se_nonlinear
        except Exception as e:
            self.att_se_nonlinear  = None  
    ...
    def forward(
        ...
        # headwise attention alignment
        self.KL_backward =torch.ones_like(key_layer).cuda().mean()
        self.MMD_backward = torch.ones_like(key_layer).cuda().mean()
        ...
        if self.training:
            ...
            if self.adver_type =='act':
                ...
                d = torch.matmul(n_x_tran, n_y_tran.transpose(-1, -2)) # mse between navigators, whats a navigator?
            ...
            if not self.config.attack:
                ...
                query_layer_skh = query_layer.reshape([-1, num_key, num_dim])
                key_layer_skh = key_layer.reshape([-1, num_key, num_dim])
                loss = SamplesLoss(loss="sinkhorn", p=2, blur=.05)
                try:
                    L = loss(query_layer_skh, key_layer_skh) # By default, use constant weights = 1/number of samples
                    self.KL_backward = L.mean().detach()
                except Exception as e:
                    print (f'couldent calculated sinkhorn loss for this batch Q:{query_layer.shape}, K:{key_layer.shape}',e)
                    self.KL_backward = self.KL_backward

        if not self.config.attack:
            mmdloss = MMD_loss().cuda()
            self.MMD_backward = mmdloss(query_layer.detach(), key_layer.detach()).mean().detach()
...
class AlbertLayer(nn.Module):
    ...
    def forward(
        ...
        self.MMD = self.attention.MMD_backward
...
class AlbertLayerGroup(nn.Module):
    ...
    def forward(
        ...
        for layer_index, albert_layer in enumerate(self.albert_layers):
            ...
            self.MMD_inner_list.append(albert_layer.MMD)
...
class AlbertTransformer(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            self.MMD_list.append(self.albert_layer_groups[group_idx].MMD_inner_list)
...
class AlbertForSequenceClassification(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        MMD = 0
        count_mmd = 0
        for inner_list in self.albert.encoder.MMD_list:
            for item in inner_list:
                MMD = MMD + item
        ...
        if labels is not None:
            ...
            # add here loss kl addition depending if we have adver_type = act or nan
            if self.config.adver_type == 'act':
                loss_total = loss+ self.config.att_kl * KL
            else:
                loss_total = loss

        if not return_dict:
            output = (logits,) + outputs[2:]
            return ((loss_total,) + output) if loss is not None else output
        ...
        # print ('loss_original', loss)
        # print ('KL_loss', KL)
        # print ('MMD_loss', MMD)
        # print ('hidden',outputs.hidden_states)
        # print ('att',outputs.attentions)
...
class AlbertForSequenceClassificationOT(AlbertPreTrainedModel):
    def __init__(self, config: AlbertConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
    ...
    def forward(
        ...
        attention_mask: Optional[torch.FloatTensor] = None,
        token_type_ids: Optional[torch.LongTensor] = None,
        # adv_input_ids: Optional[torch.LongTensor] = None,
        # adv_attention_mask: Optional[torch.FloatTensor] = None,
        # adv_token_type_ids: Optional[torch.LongTensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        head_mask: Optional[torch.FloatTensor] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None,
        ...
        # if return_dict:
        #     print (return_dict,'outputs',outputs)
            # sys.exit()
...
class AlbertF(AlbertPreTrainedModel):
    def __init__(self, config: AlbertConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
