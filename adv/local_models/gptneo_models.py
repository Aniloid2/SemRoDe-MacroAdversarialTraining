...
class SequenceClassifierOutputWithPastAndPooling(ModelOutput):
...
class CausalLMOutputWithPastAndPooling(ModelOutput):
...
class GPTNeoModel(GPTNeoPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.pooler = GPTNeoPooler(config) if add_pooling_layer else None
...
class GPTNeoForCausalLMOT(GPTNeoPreTrainedModel):
    ...
    def forward(
        ...
        return CausalLMOutputWithPastAndPooling(
            loss=loss,
            logits=lm_logits,
...
class GPTNeoForSequenceClassificationOT(GPTNeoPreTrainedModel):
    ...
    def forward(
        ...
        print ('seq len',sequence_lengths, batch_size )
        pooled_logits = logits[torch.arange(batch_size, device=logits.device), sequence_lengths]
        print ('pooled logtis',pooled_logits.shape )
        print ('actual logs', pooled_logits)
        loss = None
        if labels is not None:
        ...
        return SequenceClassifierOutputWithPastAndPooling(
            loss=loss,
            logits=pooled_logits,
            past_key_values=transformer_outputs.past_key_values,
            hidden_states=transformer_outputs.hidden_states,
            attentions=transformer_outputs.attentions,
            pooling = transformer_outputs.pooler_output,
        )
