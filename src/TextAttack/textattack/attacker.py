...
class Attacker:
    ...
    def _attack_batch_wise(self):
        ...
        worklist = [obj for obj in worklist ] 
        example_list = []
        ...
        try:
            result = self.attack.attack_batch_wise(example_list,ground_truth_output_list)
        except Exception as e:
            raise e
        ...
        for b in range(len(result)):
            ...
            if (
                isinstance(result[b], SkippedAttackResult) and self.attack_args.attack_n
            ) or (
                not isinstance(result[b], SuccessfulAttackResult)
                and self.attack_args.num_successful_examples
            ):
                if worklist_candidates:
                    next_sample = worklist_candidates.popleft()
            ...
            self.attack_log_manager.log_result(result[b])
            if not self.attack_args.disable_stdout and not self.attack_args.silent:
                print("\n")
            num_results += 1

            if isinstance(result[b], SkippedAttackResult):
                num_skipped += 1
            if isinstance(result[b], (SuccessfulAttackResult, MaximizedAttackResult)):
                num_successes += 1
            if isinstance(result[b], FailedAttackResult):
                num_failures += 1
            pbar.set_description(
                f"[Succeeded / Failed / Skipped / Total] {num_successes} / {num_failures} / {num_skipped} / {num_results}"
            )
    ...
    def attack_dataset(self):
        ...
        self.attack_args.batchwise = False
        # print (self.attack_args.batchwise)
        # sys.exit()
        ...
        elif self.attack_args.batchwise:
            
            self._attack_batch_wise()
        else:
...
# def attack_dataset_loops(self,epochs):
#     """Attack the dataset.
#
#     Returns:
#         :obj:`list[AttackResult]` - List of :class:`~textattack.attack_results.AttackResult` obtained after attacking the given dataset..
