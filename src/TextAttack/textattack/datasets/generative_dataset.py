...
def get_datasets_dataset_columns(dataset):
    ...
    if {"premise", "hypothesis", "label"} <= schema:
        ...
        # for generative modelling  we have a prompt and query
        input_columns = ("prompt","query")
        output_column = "label"
        ...
        output_column = "label"
    elif {"label", "prompt","query"} <= schema:
        input_columns = ("prompt","query")
        output_column = "label"
    else:
        raise ValueError(
...
class GenerativeDataset(HuggingFaceDataset):
    ...
    def _add_prompts(self, dataset_type = 'MR'): 
        if dataset_type == 'MR':
            prompts = ["Act as a sentiment classifier for Rotten Tomatoes movie reviews. Output 'neg' if the sentiment of the following query is negative, otherwise output 'pos' if the sentiment of the following query is positive."] * len(self._dataset)
            self._dataset = self._dataset.add_column("prompt", prompts) 
            self._dataset = self._dataset.rename_column("text", "query")
