...
class Trainer:
    ...
    def __init__(
        ...
        if self.train_dataset is not None:
            self.output_train_file = os.path.join(
                self.args.output_dir, f"train_results_{self.model.config.finetuning_task}.txt"
            )
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                extra_outputs = tr_loss_step[1]
                tr_loss_step = tr_loss_step[0]
                ...
                if self.train_dataset is not None:
                        ...
                        with open(self.output_train_file, "a") as writer:
                            ...
                            writer.write(f"Seed = {self.args.seed}, Step = {step}, Loss TR = {tr_loss_step}, Loss Original = {extra_outputs['loss_original']} , Loss KL = {extra_outputs['KL_loss']}, Loss MMD = {extra_outputs['MMD_loss']}\n")
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.args.n_gpu > 1:
            ...
            extra_losses = {}
            for (item, loss_component) in outputs.items():
                if item is not None and 'loss' in item:
                    extra_losses[item] = loss_component.mean().detach()
    ...
    def evaluation_loop(
        ...
        if all_preds is not None:
            metrics[f"{metric_key_prefix}_orig_loss"] = all_preds[1].mean().item()
            metrics[f"{metric_key_prefix}_kl_loss"] = all_preds[2].mean().item()
            metrics[f"{metric_key_prefix}_mmd_loss"] = all_preds[3].mean().item()
        ...
        output_eval_file = os.path.join(
                self.args.output_dir, f"eval_results_{self.model.config.finetuning_task}.txt"
            )
        with open(output_eval_file, "a") as writer:
            logger.info("***** Eval results {} *****".format(self.model.config.finetuning_task))
            writer.write("Seed = %s, " % (self.args.seed))
            for key, value in metrics.items():
                logger.info(" %s = %.4f", key, value)
                writer.write("%s = %.4f, " % (key, value))
            writer.write("\n")
