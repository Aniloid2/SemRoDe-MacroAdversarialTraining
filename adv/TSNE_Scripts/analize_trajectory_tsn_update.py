...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_attack")
    parser.add_argument("--model_plotting")
    parser.add_argument("--name_attack")
    parser.add_argument("--name_plotting")
    ...
    if '/BERT/' in args.model_attack or "bert-base-uncased" in args.model_attack :
        Model_save = 'BERT'
    elif '/ALBERT/' in args.model_attack or "albert-base-v2" in args.model_attack :
        Model_save = 'ALBERT'
    ...
    # else:
    #     M_Type = 'AdvT'
    M_Type = f'{args.name_attack}_{args.name_plotting}'

    textattack.shared.utils.set_seed(0)

    ID = f'{args.name_attack}_{args.name_plotting}'+'_'+'D'+args.Dataset+'_'+'B'+str_B_Val+'_'+'AT'+str_AT_Val+'_'+ 'OT'+str_OT_Val +'_'+ 'MMD'+str_MMD_Val+'_' +'CRL'+str_CRL_Val + '_'+'DR'+str_Data_ratio+'_'+'E'+str_Epochs +'_'+'LR'+str_LR+'_'+'NWS'+str_NWS +'_'+'F'+ str_F
    ...
    origin_folder_attack = args.model_attack
    origin_folder_plotting = args.model_plotting
    ...
    if 'Raw' in args.id:
        ...
        origin_folder_attack = args.model_attack # f"textattack/bert-base-uncased-rotten-tomatoes"
        origin_folder_plotting = args.model_plotting # f"textattack/bert-base-uncased-rotten-tomatoes"


        ID = f'{args.name_attack}_{args.name_plotting}'+'_'+'E'+str_Epochs
        output_folder = f"./Pictures/{args.Dataset}/{Model_save}/{ID}"
        output_name = ID
    ...
    model_attack = local_models.BertForSequenceClassificationOT.from_pretrained(origin_folder_attack)

    attack_bool = {"attack":True}

    model_attack.config.update(attack_bool)
    print ('model_attack configuration',model_attack.config)
    tokenizer = transformers.AutoTokenizer.from_pretrained(origin_folder_attack,model_max_length=128)
    ...
    model_wrapper_attack = HuggingFaceModelWrapper(model_attack, tokenizer)
    ...
    if args.Dataset == 'MRPC':
        ...
        base_dataset = textattack.datasets.HuggingFaceDataset('ag_news', None,'test[250:500]+test[-500:-250]')
        pre_adv_dataset = textattack.datasets.HuggingFaceDataset('ag_news', None,'test[250:500]+test[-500:-250]')
        c0 = 0
        c1 = 0
        c2 = 0
        c3 = 0
        for i,j in enumerate(pre_adv_dataset):
            ...
            if j[-1] == 0:
                ...
                c3+=1
        print ('pre_adv_dataset distribution','len eval',len(pre_adv_dataset),'c0',c0,'c1' , c1, 'c2' ,c2,'c3',c3)
    ...
    attack = textattack.attack_recipes.TextFoolerJin2019.build(model_wrapper_attack,max_modification_rate=0.3)
    ...
    if my_file.is_file():
        ...
        for i,r in enumerate(results):
            if isinstance(r, (SuccessfulAttackResult, MaximizedAttackResult)):
                ...
                list_num_words_changed.append((i,num_words_changed))
                # if num_words_changed < 1 or num_words_changed >= 5:
                #     continue
                
                if i != number_analysing:# 299:# 494:#299:
                    continue
                else:
                    ...
                    for i,entry in enumerate(extended_results):
                        print ('entry',i,entry)
                    break

        print ('indicies, succ attacks',list_num_words_changed)        
    ...
    if origin_folder_attack == origin_folder_plotting :
        model_plotting = model_attack
        
    else:
        
        model_plotting = local_models.BertForSequenceClassificationOT.from_pretrained(origin_folder_plotting)

        attack_bool = {"attack":True}

        model_plotting.config.update(attack_bool)
        print ('model_plotting configuration',model_plotting.config)
        tokenizer = transformers.AutoTokenizer.from_pretrained(origin_folder_plotting,model_max_length=128)
 

        model_wrapper_plotting = HuggingFaceModelWrapper(model_plotting, tokenizer)
        
    model_plotting.eval()
    model_plotting.to(textattack.shared.utils.device)
    ...
    for step, base_batch in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            base_output = model_plotting(**base_input_ids ,return_dict=True,output_hidden_states=True) # need to extract last features
    ...
    for step, adv_batch in enumerate(adv_dataloader):
        ...
        with torch.no_grad():
            adv_output = model_plotting(**adv_input_ids ,return_dict=True,output_hidden_states=True) # need to extract last features
    ...
    for step, progress_batch in enumerate(progress_dataloader):
        ...
        with torch.no_grad():
            progress_output = model_plotting(**progress_input_ids ,return_dict=True,output_hidden_states=True) # need to extract last features

        progress_logits = progress_output[0]
        progress_pooler_output = progress_output.pooler_output

        print ('logits progress',progress_logits)
    ...
    if args.Dataset == 'MR':
        ...
        prev_targets_np = prev_targets.cpu().numpy()+4
    ...
    if args.Dataset == 'MR':
        ...
        progress_prev_targets_np = progress_prev_targets.cpu().numpy() + 16
    ...
    if args.Dataset == 'MR':
        ...
        legend_map = {0:'Base Sample (0)',1:'Base Sample (1)',2:'Base Sample (2)', 3:'Base Sample (3)'
                      ,4:'Successful Adv Sample (0->1)',5:'Successful Adv Sample (0->2)',6:'Successful Adv Sample (0->3)'
                      ,7:'Successful Adv Sample (1->0)',8:'Successful Adv Sample (1->2)',9:'Successful Adv Sample (1->3)',
                      10:'Successful Adv Sample (2->0)',11:'Successful Adv Sample (2->1)',12:'Successful Adv Sample (2->3)',
                      13:'Successful Adv Sample (3->0)',14:'Successful Adv Sample (3->1)',15:'Successful Adv Sample (3->2)',
                      
                      
                      }

                    #   10:'Progression Sample (2->0)',11:'Progression Sample (2->1)',12:'Progression Sample (2->3)',
                    #   13:'Progression Sample (3->0)',14:'Progression Sample (3->1)',15:'Progression Sample (3->2)',
                      
        # color_dictionary = {'Base Sample (0)':"#ADD8E6", "Base Sample (1)":"#FFFF00", 'Successful Adv Sample (0->1)':"#0000FF",'Successful Adv Sample (1->0)':"#FF0000",'Progression Sample (1->0)':"#000000",'Progression Sample (0->1)':"#000000"}
        # color_dictionary = {0:"#ADD8E6", 1:"#FFFF00", 2:"#0000FF",3:"#FF0000",5:"#000000",6:"#FFC0CB"}
        color_dictionary = {'Base Sample (0)':"#0000FF", # Blue
                            "Base Sample (1)":"#DC143C", #Crimson
                            'Base Sample (2)':"#FFD700", # Blue
                            "Base Sample (3)":"#008000", #green
                            'Successful Adv Sample (0->1)':"#87cefa", #lightskyblue
                            'Successful Adv Sample (0->2)':'#87cefa', #Gold
                            'Successful Adv Sample (0->3)':"#87cefa", #lightskyblue 
                            'Successful Adv Sample (1->0)':"#87cefa", #lightskyblue
                            'Successful Adv Sample (1->2)':'#87cefa', #Gold
                            'Successful Adv Sample (1->3)':"#87cefa", #lightskyblue 
                            'Successful Adv Sample (2->0)':"#87cefa", #lightskyblue
                            'Successful Adv Sample (2->1)':'#87cefa', #Gold
                            'Successful Adv Sample (2->3)':"#87cefa", #lightskyblue 
                            'Successful Adv Sample (3->0)':"#87cefa", #lightskyblue
                            'Successful Adv Sample (3->1)':'#FFD700', #Gold
                            'Successful Adv Sample (3->2)':"#87cefa", #lightskyblue  
                            }
    ...
    # two_samples = df.iloc[[-len_progression,-1]]
    # print (two_samples)
    # sns.scatterplot(data=two_samples, x="x", y="y",alpha=two_samples['Alpha'],hue=two_samples['Sample Type'].map(legend_map),palette=color_dictionary,ax=ax1)
    ...
    if args.Arrow:
        ax1.plot(progression_x[:-1], progression_y[:-1], color='black')
        ax1.annotate("", xy=(progression_x[-1], progression_y[-1]), xytext=(progression_x[-2],progression_y[-2]),
                arrowprops=dict(arrowstyle="->"))
        # for i in range(0,len_progression-1):
        #     ax1.annotate("", xy=(progression_x[-1], progression_y[-1]), xytext=(progression_x[i],progression_y[i]),
        #         arrowprops=dict(arrowstyle="->"))
    ...
    # if args.Dataset == 'MR':
    #     if number_analysing < 250:
    #         example_color_dictionary = color_dictionary
    #         example_color_dictionary.pop('Progression Sample (0->1)')
    #     else:
    #         example_color_dictionary = color_dictionary
    #         example_color_dictionary.pop('Progression Sample (1->0)')
    # else:
    example_color_dictionary = color_dictionary
    handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=color, markersize=10, label=label)
           for label, color in example_color_dictionary.items()]

    l = ax1.legend(handles = handles, title='Sample\'s Type',loc='upper right', bbox_to_anchor=(0.5, 0.5, 1, 1))
    l.set_zorder(4)
    ...
    fig1.savefig(f'./Pictures/TSNE/{args.Dataset}/{Model_save}/{M_Type}_tsn_DS{args.Dataset}_DR{args.Data_ratio}_MinW{args.Min_Words}_MaxW{args.Max_Words}_Sing_ARW{args.Arrow}{number_analysing}.png',bbox_inches='tight')
    fig1.savefig(f'./Pictures/TSNE/{args.Dataset}/{Model_save}/{M_Type}_tsn_DS{args.Dataset}_DR{args.Data_ratio}_MinW{args.Min_Words}_MaxW{args.Max_Words}_Sing_ARW{args.Arrow}{number_analysing}.eps',bbox_inches='tight')
    ...
    plot2 = sns.jointplot(data = df[:-len_progression], x = 'x', y = 'y',hue=df.iloc[:-len_progression]['Sample Type'].map(legend_map), kind = 'kde',palette=color_dictionary, legend='full',zorder =2)
    ...
    if args.Arrow:
        # two_samples = df.iloc[[-len_progression,-1]]
        # print (two_samples)
        # sns.scatterplot(data=two_samples, x="x", y="y",hue=two_samples['Sample Type'].map(legend_map),palette=color_dictionary,ax=ax2,zorder =3)
 
        sns.scatterplot(data=df.iloc[-len_progression:], x="x", y="y",hue=df.iloc[-len_progression:]['Sample Type'].map(legend_map),palette=color_dictionary,ax=ax2,zorder =3, legend='full')


        ax2.plot(progression_x[:-1], progression_y[:-1], color='black')

        ax2.annotate("", xy=(progression_x[-1], progression_y[-1]), xytext=(progression_x[-2],progression_y[-2]),
                arrowprops=dict(arrowstyle="->"))
    ...
    l = ax2.legend(handles = handles, title='Sample\'s Type',loc='upper right', bbox_to_anchor=(0.5, 0.55, 1, 1))
    l.set_zorder(4)

    if 'robust' in M_Type:# == 'AdvT':
        ax2.set_title(f'Robust T-SNE\nAdv Samples {len(adv_dataset)} ' , y=1.25)
    elif 'base' in  M_Type:# == 'BaseT':
        ax2.set_title(f'Non-Robust T-SNE\nAdv Samples {len(adv_dataset)} ' , y=1.25)

    # sns.scatterplot(data=df, x="x", y="y",hue="l",palette=["C0", "C1", "C2","C3"])
    plot2.savefig(f'./Pictures/TSNE/{args.Dataset}/{Model_save}/{M_Type}_tsn_DS{args.Dataset}_DR{args.Data_ratio}_MinW{args.Min_Words}_MaxW{args.Max_Words}_ARW{args.Arrow}{number_analysing}.png')
    plot2.savefig(f'./Pictures/TSNE/{args.Dataset}/{Model_save}/{M_Type}_tsn_DS{args.Dataset}_DR{args.Data_ratio}_MinW{args.Min_Words}_MaxW{args.Max_Words}_ARW{args.Arrow}{number_analysing}.eps')
    ...
    plot3 = sns.jointplot(data = df[:-len_progression], x = 'x', y = 'y',hue=df[:-len_progression]['Sample Type'].map(legend_map), kind = 'kde', palette=color_dictionary)
    ax3 = plot3.ax_joint
    ...
    if args.Arrow:
        ...
        ax3.plot(progression_x[:-1], progression_y[:-1], color='black')

        ax3.annotate("", xy=(progression_x[-1], progression_y[-1]), xytext=(progression_x[-2],progression_y[-2]),
                arrowprops=dict(arrowstyle="->"))
    ...
    plot3.savefig(f'./Pictures/TSNE/{args.Dataset}/{Model_save}/{M_Type}_tsn_DS{args.Dataset}_DR{args.Data_ratio}_MinW{args.Min_Words}_MaxW{args.Max_Words}_Joint_ARW{args.Arrow}{number_analysing}.png')
    plot3.savefig(f'./Pictures/TSNE/{args.Dataset}/{Model_save}/{M_Type}_tsn_DS{args.Dataset}_DR{args.Data_ratio}_MinW{args.Min_Words}_MaxW{args.Max_Words}_Joint_ARW{args.Arrow}{number_analysing}.eps')
